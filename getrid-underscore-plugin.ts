/**
 * source: https://github.com/Lazyuki/rollup-plugin-rename-node-modules
 * license: MIT license (c) 2020 Lazyuki
 */
...
const plugin = (
  ...
  const replace =
    typeof moduleName === "string"
      ? (fileName: string) => fileName.replace(/_/g, '')
      : moduleName;
  return {
    ...
    generateBundle(_, bundle) {
      ...
      Object.entries(bundle).forEach(([fileName, chunkInfo]) => {
        ...
        if ("code" in chunkInfo) {
          if (chunkInfo.imports.some((i) => i.includes("_"))) {
